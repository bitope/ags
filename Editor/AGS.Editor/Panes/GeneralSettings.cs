...
namespace AGS.Editor
{
    public partial class GeneralSettings : EditorContentPanel
    {
        ...
        private void HandleGameResolutionChange(Size oldResolution, Size newResolution)
        {
            ...
            if (Factory.GUIController.ShowQuestion(string.Format("You've changed your game resolution from {0} to {1}.{2}You will need to import a new background of the correct size for all your rooms.{2}{2}Would you like AGS to automatically resize all your GUIs to the new resolution?", oldResolution.ToString(), newResolution.ToString(), Environment.NewLine)) == DialogResult.Yes)
            {
        ...
        private void gameSettings_PropertyValueChanged(object s, PropertyValueChangedEventArgs e)
        {
            ...
            {
                GameResolutions newResolution = Factory.AGSEditor.CurrentGame.Settings.Resolution;
                GameResolutions oldResolution = (GameResolutions)e.OldValue;

                //  Ask the user before changing to large resolutions
                if ((newResolution == GameResolutions.R800x600 || newResolution == GameResolutions.R1024x768) && 
                    Factory.GUIController.ShowQuestion("Are you sure you need to use this resolution? High resolutions like 800x600 and 1024x768 increase the file size of your game and increase the system requirements needed to play it. Are your graphics really detailed enough to need this?") == DialogResult.No)
                {
                    Factory.AGSEditor.CurrentGame.Settings.Resolution = oldResolution;
                    return;
                }

                //  Handle resolution change using proper dimensions
                HandleGameResolutionChange(
                    oldResolution == GameResolutions.CustomResolution ? Factory.AGSEditor.CurrentGame.Settings.CustomResolution : AGS.Types.Utilities.GetGameResolution(oldResolution),
                    newResolution == GameResolutions.CustomResolution ? Factory.AGSEditor.CurrentGame.Settings.CustomResolution : AGS.Types.Utilities.GetGameResolution(newResolution)
                );

                //  Set the proper Letterbox mode (this property is ignored in CustomResoution mode)
                Factory.AGSEditor.CurrentGame.Settings.LetterboxMode = (newResolution == GameResolutions.R320x240) || (newResolution == GameResolutions.R640x480);
            }
            else if (e.ChangedItem.Label == AGS.Types.Settings.PROPERTY_RESOLUTION_CUSTOM || (e.ChangedItem.Parent != null && e.ChangedItem.Parent.Label == AGS.Types.Settings.PROPERTY_RESOLUTION_CUSTOM))
            {
                //  Only adjust the game resolution if the game actually is in CustomResolution mode
                if (((Settings)gameSettings.SelectedObject).Resolution == GameResolutions.CustomResolution)
